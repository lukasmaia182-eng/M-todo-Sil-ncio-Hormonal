...
export default function SeuAcessoEbookPage() {
  ...
  if (!cap) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Link href="/seuacesso" className="text-sm text-[var(--color-brand)] hover:underline">
          Voltar ao início
        </Link>
      </div>
    )
  ...
  return (
    <div className="flex flex-col gap-5 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl p-5 border border-[var(--color-border)] shadow-sm">
        ...
        <div className="mt-3">
          <div className="relative h-1.5 rounded-full bg-[var(--color-muted)] overflow-hidden">
            <div className="h-full rounded-full bg-[var(--color-brand)]" style={{ width: `${(cap.chapter / ebookCapitulos.length) * 100}%` }} />
          </div>
          <p className="text-xs text-[var(--color-muted-foreground)] mt-1">{cap.chapter}/{ebookCapitulos.length} capítulos</p>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        {prevCap && (
          <Link href={`/seuacesso/ebook/${prevCap}`} className="flex-1 flex items-center justify-center gap-2 rounded-2xl border border-[var(--color-border)] bg-white text-sm font-semibold text-[var(--color-dark)] py-3 hover:border-[var(--color-brand)]/40 hover:shadow-sm transition-all">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
            Cap. {prevCap}
          </Link>
        )}
        {nextCap && (
          <Link href={`/seuacesso/ebook/${nextCap}`} className="flex-1 flex items-center justify-center gap-2 rounded-2xl bg-[var(--color-brand)]/10 text-[var(--color-brand)] text-sm font-semibold py-3 hover:bg-[var(--color-brand)]/20 transition-all">
            Cap. {nextCap}
